  ...
  return (
    <div className="grid grid-cols-12 lg:px-10 md:px-4 px-3 py-2 bg-gray-800 text-white font-qsand">
      ...
      <i
        onClick={handleMenu}
        ref={btn}
        className="fas fa-bars md:hidden col-span-10 sm:text-3xl text-2xl text-right"
      ></i>
      <nav
        ref={menu}
        className="md:col-span-10 md:flex hidden col-span-12 items-center"
      >
        <div className="grid grid-cols-10 w-full">
          <div className="md:col-span-6 col-span-10">
            <div className="flex md:flex-row flex-col md:justify-start justify-center items-center">
              <NavLink
                to="/"
                className="md:text-lg text-base font-medium  md:mr-4"
              >
                Home
              </NavLink>
              <NavLink
              ...
              {user?.email ? (
                <span className="md:inline flex flex-col text-center">
                  <img
                    src={user.photoURL}
                    alt=""
                    className="w-10 h-10 mx-auto rounded-full md:inline-block block"
                  />
                  <h5 className="md:inline-block block px-2 md:text-sm lg:text-base">
                    {name}
                  </h5>
                  <button
                    onClick={logOut}
                    className="bg-coral text-white rounded px-4 py-0.5"
                  >
                    Logout
                  </button>
                </span>
